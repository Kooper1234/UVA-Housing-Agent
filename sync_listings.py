...
def fetch_rentcast_listings(
    city: str = "Charlottesville", state: str = "VA", limit: int = 100
):
    """
        ...
        async with conn.transaction():
            ...
            for listing in listings:
                try:
                    ...
                    print(
                        f"Error upserting listing {listing.get('id', 'unknown')}: {e}"
                    )
                    skipped += 1
...
async def main():
    ...
    api_listings = fetch_rentcast_listings(
        city="Charlottesville", state="VA", limit=100
    )
    ...
    print(
        f"Successfully transformed {len(transformed)} listings (skipped {len(api_listings) - len(transformed)} with missing fields)\n"
    )
